...
describe('mdata:patch', () => {
  ...
  describe('patch workflows', () => {
    let writeFileSyncStub;
    const commonStubs = function () {
      stubMethod($$.SANDBOX, glob, 'glob').callsFake((pattern: string, cb: (err: Error | null, matches: string[]) => void) => {
        cb(null, [path.join('force-app', 'main', 'default', 'workflows', 'Order.workflow-meta.xml')])
      })
      const readFileSyncStub = stubMethod($$.SANDBOX, fs, 'readFileSync')
    ...
    test
          ...
          return {
            ...
            "plugins": {
              "mdataPatches": {
                "devShared": {
                  "workflows/*": {
                    "where": "Workflow.alerts",
                    "replace": {
                      "senderType": "OrgWideEmailAddress"
                    },
                    "concat": [{ senderAddress: "sample.email@example.com" }]
                  }
                }
              }
            }
      ...
      .stdout()
      .command(['mdata:patch', '-e', 'devShared'])
      .it('runs mdata:patch on Workflows by changing sender address on multiple Email Alerts', ctx => {
        expect(writeFileSyncStub.args[0][0]).to.equal(path.join('force-app', 'main', 'default', 'workflows', 'Order.workflow-meta.xml'));
        expect(writeFileSyncStub.args[0][1]).to.have.entriesCount(`<senderType>OrgWideEmailAddress</senderType>`, 2);
        expect(writeFileSyncStub.args[0][1]).to.have.entriesCount(`<senderAddress>sample.email@example.com</senderAddress>`, 2);
      });
  });
