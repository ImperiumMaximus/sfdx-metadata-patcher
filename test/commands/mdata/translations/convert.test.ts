...
import Convert from '../../../../src/commands/mdata/translations/convert';
...
describe('mdata:translations:convert', () => {
    ...
    describe('Invalid "from" & "to" combo', () => {
        it('should return an error message', async () => {
            ...
            try { await Convert.run(['-f', 'xlsx', '-t', 'xlsx', '-i', 'invalid.zip', '-d', 'invalid.xlsx']) } catch (err) { }
            const output = sfCommandStubs.logToStderr
                .getCalls()
                .flatMap((c) => c.args)
                .join('\n');
            expect(output).to.contain(messages.getMessage('translations.convert.errors.invalidFromToCombo'));
        });
    });
    ...
    describe('Import single stf file', () => {
        ...
        it('should generate an xlsx file with one sheet', async () => {
            ...
            await Convert.run(['-f', 'stf', '-t', 'xlsx', '-i', 'Outdated and untranslated_en_US.stf', '-d', 'out.xlsx'])
    ...
    describe('Import multiple stf files in a zip', () => {
        let xlsxWriteFileStub: sinon.SinonStub;
        let workbook: ExcelJS.Workbook;
        it('should generate an xlsx file with two sheets', async () => {
            const existsSyncStub = stubMethod($$.SANDBOX, fs, 'existsSync')
            existsSyncStub.callsFake((p: string) => {
            ...
            await Convert.run(['-f', 'stf', '-t', 'xlsx', '-i', 'Outdated and untranslated.zip', '-d', 'out.xlsx'])
    ...
    describe('Import single stf Bilingual file', () => {
        let xlsxWriteFileStub: sinon.SinonStub;
        let workbook: ExcelJS.Workbook;
        it('should generate an xlsx file with one sheet and the translated string at the top', async () => {
            const existsSyncStub = stubMethod($$.SANDBOX, fs, 'existsSync')
            ...
            await Convert.run(['-f', 'stf', '-t', 'xlsx', '-i', 'Bilingual_it.stf', '-d', 'out.xlsx'])
    ...
    describe('Import single stf Source file', () => {
        let xlsxWriteFileStub: sinon.SinonStub;
        let workbook: ExcelJS.Workbook;
        it('should generate an xlsx file with one sheet with Translation and Out of Date columns having unknown status', async () => {
            const existsSyncStub = stubMethod($$.SANDBOX, fs, 'existsSync')
            existsSyncStub.callsFake((_path: string) => {
            ...
            xlsxWriteFileStub = stubMethod($$.SANDBOX, ExcelJS.Workbook.prototype.xlsx, 'writeFile');
            await Convert.run(['-f', 'stf', '-t', 'xlsx', '-i', 'Source_en_US.stf', '-d', 'out.xlsx'])
    ...
    describe('Import excel file with single sheet', () => {
        ...
        it('should generate a single STF file', async () => {
            ...
            await Convert.run(['-f', 'xlsx', '-t', 'stf', '-i', path.join(__dirname, '..', '..', '..', 'data', 'translations', 'Bilingual_it.xlsx'), '-d', 'outdir'])
    ...
    describe('Import excel file with multiple sheet', () => {
        ...
        it('should generate a ZIP file with the same number of files', async () => {
            // Workaround in order to have a consistent instance of an ExcelJS.Workbook.
            // For some reason the instance created in the actual code doesn't have
            // its properties correctly populated, though this happens only during
            // test execution.
            ...
            await Convert.run(['-f', 'xlsx', '-t', 'stf', '-i', path.join(__dirname, '..', '..', '..', 'data', 'translations', 'Bilingual_en_US_it.xlsx'), '-d', 'outdir'])
    ...
    describe('Import excel file with multiple sheets but filtering', () => {
        ...
        it('should generate a single STF file', async () => {
            ...
            await Convert.run(['-f', 'xlsx', '-t', 'stf', '-i', path.join(__dirname, '..', '..', '..', 'data', 'translations', 'Bilingual_en_US_it.xlsx'), '-d', 'outdir', '-s', 'en_US'])
